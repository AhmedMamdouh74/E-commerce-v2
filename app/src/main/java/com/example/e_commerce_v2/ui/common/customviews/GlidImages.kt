package com.example.e_commerce_v2.ui.common.customviews
...
//        .signature(Object())
        .transform(CenterCrop(), RoundedCorners(16)).into(view)
}
...
fun categoryUrl(view: ImageView, imageUrl: String?) {
    ...
    Glide.with(view.context).load(imageUrl).placeholder(loading)
        .diskCacheStrategy(DiskCacheStrategy.ALL).transform(CenterInside()).into(view)
}
...
fun circleNormalImage(view: ImageView, imageUrl: String?) {
    val loading = getGlideCircleLoading(view)
    Glide.with(view.context).load(imageUrl).placeholder(loading)
        .diskCacheStrategy(DiskCacheStrategy.ALL).transform(CircleCrop()).into(view)
}
